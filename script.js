...
const CONFIG = {
    ...
    // 3. จัดซื้อ
    email_Step3_Purchasing: 'hr.bpp.2564@gmail.com',
    // --------------------------------------------------------

    // รหัสผ่านแยกตามตำแหน่ง
    passwords: {
        'manager': '1111',      // รหัส คุณศุภรัตน์ (Manager)
        'assist_md': '9999'     // รหัส คุณเบญจมาศ (Assistant MD)
    }
};
...
// Login Function (แยก Role ชัดเจน)
window.checkAdminPassword = function() {
    const input = document.getElementById('adminPassInput').value;
    
    if (input === CONFIG.passwords.manager) {
        // ล็อกอินเป็น คุณศุภรัตน์
        currentUserRole = 'manager'; 
        sessionStorage.setItem('userRole', 'manager'); 
        sessionStorage.setItem('isAdmin', 'true');
    } else if (input === CONFIG.passwords.assist_md) {
        // ล็อกอินเป็น คุณเบญจมาศ
        currentUserRole = 'assist_md'; 
        sessionStorage.setItem('userRole', 'assist_md'); 
        sessionStorage.setItem('isAdmin', 'true');
    } else {
...
function updateAdminUI() {
    ...
    if (title) {
        if (currentUserRole === 'manager') title.innerText = '👑 รายการรอตรวจสอบ (ผู้จัดการ: คุณศุภรัตน์)';
        else if (currentUserRole === 'assist_md') title.innerText = '👑 รายการรออนุมัติ (ผู้ช่วย กก.: คุณเบญจมาศ)';
    }
}

// ================= PART 1: FORM SUBMIT =================
window.addItemRow = function() { /* ... (โค้ด addItemRow เดิม ใช้ของเก่าได้เลยครับ ไม่เปลี่ยน) ... */ 
    ...
    prForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            const payload = {
                ...
                // [จุดเปลี่ยนสำคัญ] ส่งให้ Manager (ศุภรัตน์) ก่อน
                status: 'wait_manager' 
            };
            ...
            await emailjs.send(CONFIG.emailServiceId, CONFIG.emailTemplateId_Master, {
                to_email: CONFIG.email_Step1_Manager,
                subject: `[New Request] ขอตรวจสอบ PR ${payload.pr_number}`,
                html_content: `
                    <h3>เรียน ผู้จัดการ (คุณศุภรัตน์),</h3>
                    <p>มีรายการขอซื้อใหม่จาก <b>${payload.requester}</b> รอการตรวจสอบครับ</p>
                    <p>เลขที่ PR: ${payload.pr_number}</p>
                    <p><a href="${adminLink}">คลิกเพื่อเข้าสู่ระบบ</a></p>
                `
            });

            alert('✅ ส่งเรื่องถึงผู้จัดการ (คุณศุภรัตน์) เรียบร้อยแล้ว!');
...
async function loadPRs() {
    ...
    try {
        ...
        if (currentMode === 'pending') {
            // กรองงานตามตำแหน่ง (Role)
            if (currentUserRole === 'manager') {
                query = query.eq('status', 'wait_manager'); // ศุภรัตน์ เห็นงาน wait_manager
            } else if (currentUserRole === 'assist_md') {
                query = query.eq('status', 'wait_assist_md'); // เบญจมาศ เห็นงาน wait_assist_md
            }
        } else {
            // History
            if (currentUserRole === 'manager') query = query.neq('status', 'wait_manager');
            else query = query.in('status', ['finished', 'approved', 'rejected']);
        }
        ...
        data.forEach(pr => {
            ...
            if (pr.status === 'wait_manager') statusText = 'รอ ผจก. ตรวจสอบ';
            if (pr.status === 'wait_assist_md') statusText = 'รอ ผช.กก. อนุมัติ';
            if (pr.status === 'finished') statusText = 'อนุมัติเรียบร้อย';

            const row = `
                <tr>
                    <td><span class="fw-bold text-primary">${pr.pr_number}</span></td>
                    <td>${createdDate}</td>
                    <td><div class="fw-bold">${pr.requester}</div><small class="text-muted">${pr.department}</small></td>
...
window.openDetailModal = function(id) {
    ...
    if (currentMode === 'history') {
        ...
        // เปลี่ยนชื่อปุ่มตามตำแหน่ง
        saveBtn.innerText = (currentUserRole === 'manager') ? 'ส่งต่อให้ผู้ช่วย กก. ➡️' : '✅ อนุมัติและส่งเมล';
    }

    new bootstrap.Modal(document.getElementById('detailModal')).show();
}

// (ฟังก์ชัน renderItemsTable, toggleSelectAll, toggleItem ใช้ของเดิมได้เลยครับ)
// ... แต่เพื่อความชัวร์ ผมแปะให้ครบ ...
function renderItemsTable() {
    ...
    if(currentMode === 'history') {
        itemsTable.querySelectorAll('input, select').forEach(el => el.disabled = true);
        if(document.getElementById('selectAll')) document.getElementById('selectAll').disabled = true;
    }
}
...
// ================= 3. FINAL APPROVAL LOGIC (สำคัญสุด) =================
window.finalizeApproval = async function() {
    ...
    checkboxes.forEach(cb => {
        ...
        if (cb.checked) { currentPR.items[idx].status = 'approved'; currentPR.items[idx].remark = ''; } 
            ...
            if (!currentPR.items[idx].remark) { hasRejectedWithoutReason = true; reasonInput.classList.add('is-invalid'); } 
        }
    });
    ...
    try {
        ...
        // --- 1. ถ้าเป็น Manager (คุณศุภรัตน์) ---
        if (currentUserRole === 'manager') {
            nextStatus = 'wait_assist_md'; // ส่งต่อให้ Assist MD
            
            await db.from('purchase_requests').update({ status: nextStatus, items: currentPR.items }).eq('id', currentPR.id);
            
            // ส่งเมลหา Assist MD (คุณเบญจมาศ)
            await emailjs.send(CONFIG.emailServiceId, CONFIG.emailTemplateId_Master, {
                to_email: CONFIG.email_Step2_AssistMD,
                subject: `[Step 2] ผ่านการตรวจสอบแล้ว รออนุมัติ PR ${currentPR.pr_number}`,
                html_content: `
                    <h3>เรียน ผู้ช่วยกรรมการผู้จัดการ (คุณเบญจมาศ),</h3>
                    <p>คุณศุภรัตน์ (ผู้จัดการ) ได้ตรวจสอบ PR เลขที่ <b>${currentPR.pr_number}</b> เรียบร้อยแล้ว</p>
                    <p>กรุณาพิจารณาอนุมัติขั้นตอนสุดท้าย: <a href="${adminLink}">คลิกที่นี่</a></p>
                `
            });
            alert('✅ บันทึกผลแล้ว ส่งต่อให้คุณเบญจมาศเรียบร้อย!');
        } 
        
        // --- 2. ถ้าเป็น Assist MD (คุณเบญจมาศ) ---
        else if (currentUserRole === 'assist_md') {
            nextStatus = 'finished'; // จบงาน
            ...
            currentPR.items.forEach(i => {
                ...
                if (i.status === 'approved') {
                    ...
                    approvedTable += `<tr><td style="border:1px solid #ddd;padding:5px;">${i.code||'-'}</td><td style="border:1px solid #ddd;padding:5px;">${i.description}</td><td style="border:1px solid #ddd;padding:5px;">${i.quantity}</td></tr>`;
                }
            });
            ...
            // ส่งเมลหา จัดซื้อ
            if (hasApprovedItems && CONFIG.email_Step3_Purchasing) {
                await emailjs.send(CONFIG.emailServiceId, CONFIG.emailTemplateId_Master, {
                    to_email: CONFIG.email_Step3_Purchasing,
...
async function loadPRForPrint() {
    ...
    try {
        ...
        // แสดงสถานะ
        let statusTxt = 'รออนุมัติ';
        if (pr.status === 'finished') statusTxt = 'อนุมัติเรียบร้อย';
        else if (pr.status === 'wait_manager') statusTxt = 'รอ ผจก. ตรวจสอบ';
        else if (pr.status === 'wait_assist_md') statusTxt = 'รอ ผช.กก. อนุมัติ';
        document.getElementById('v_doc_status').innerText = statusTxt;

        // ลายเซ็น (เช็คจากสถานะ)
        if (pr.status === 'wait_assist_md' || pr.status === 'finished') {
        ...
        if (displayItems) {
            displayItems.forEach((item, index) => {
                ...
                else if (item.status === 'rejected') statusText = `<span style="text-decoration:line-through;color:red;">❌ ไม่อนุมัติ</span>`;
...
// if(document.getElementById('prTableBody')) window.onload = loadPRs; // ไม่ต้อง
