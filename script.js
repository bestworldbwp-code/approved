...
const CONFIG = {
    // Supabase (ใช้ URL จากรูปที่คุณส่งมา)
    supaUrl: 'https://pufddwdcpugilwlavban.supabase.co', 
    supaKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InB1ZmRkd2RjcHVnaWx3bGF2YmFuIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTkzODY1MDUsImV4cCI6MjA3NDk2MjUwNX0.6dyYteDu6QSkTL9hIiaHw_2WeltSGSIoMSvx3OcEjN0', // ⚠️ ใส่ Key ยาวๆ ของคุณตรงนี้ (eyJh...)
    
    // EmailJS (ใช้ค่าจากรูปที่คุณส่งมา)
    emailPublicKey: 'rEly1Il6Xz0qZwaSc',   
    emailServiceId: 'service_tolm3pu',   
    emailTemplateId_Master: 'template_master', 

    // --------------------------------------------------------
    // [แก้ไขแล้ว] อีเมลผู้รับ (ตามที่คุณแจ้งล่าสุด)
    // --------------------------------------------------------
    bossEmail: 'jakkidmarat@gmail.com',          // Boss (หัวหน้า/ผู้ตรวจสอบ)
    managerEmail: 'bestworld.bwp328@gmail.com',  // กรรมการ (ผู้อนุมัติ)
    purchasingEmail: 'hr.bpp.2564@gmail.com',    // จัดซื้อ (แก้เพิ่มจุด . แล้ว)

    // รหัสผ่าน
    passwords: {
        'head': '1111',    // รหัสสำหรับ jakkidmarat (Boss)
        'manager': '9999'  // รหัสสำหรับ bestworld (กรรมการ)
    }
};
...
function updateAdminUI() {
    ...
    if (title) {
        if (currentUserRole === 'head') title.innerText = '👑 รายการรอตรวจสอบ (คุณจักรกฤษณ์)';
        else if (currentUserRole === 'manager') title.innerText = '👑 รายการรออนุมัติ (กรรมการ)';
    }
}
...
if (prForm) {
    prForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // ส่งหา Boss (jakkidmarat)
            await emailjs.send(CONFIG.emailServiceId, CONFIG.emailTemplateId_Master, { to_email: CONFIG.bossEmail, subject: `[New Request] ขอตรวจสอบ PR ${payload.pr_number}`, html_content: `<h3>เรียน หัวหน้างาน (คุณจักรกฤษณ์),</h3><p>มีรายการขอซื้อใหม่จาก <b>${payload.requester}</b> รอการตรวจสอบครับ</p><p>เลขที่ PR: ${payload.pr_number}</p><p><a href="${adminLink}">คลิกเพื่อเข้าสู่ระบบ</a></p>` });
...
async function loadPRs() {
    ...
    try {
        ...
        data.forEach(pr => {
            ...
            if (pr.status === 'pending_head') statusText = 'รอหัวหน้าตรวจสอบ';
            if (pr.status === 'pending_manager') statusText = 'รอกรรมการอนุมัติ';
...
window.openDetailModal = function(id) {
    ...
    if (currentMode === 'history') { saveBtn.style.display = 'none'; } 
        ...
        saveBtn.innerText = (currentUserRole === 'head') ? 'ส่งต่อให้กรรมการ ➡️' : '✅ อนุมัติและส่งเมล';
    }
...
window.finalizeApproval = async function() {
    ...
    try {
        ...
        if (currentUserRole === 'head') {
            ...
            // ส่งหา กรรมการ (bestworld)
            await emailjs.send(CONFIG.emailServiceId, CONFIG.emailTemplateId_Master, { to_email: CONFIG.managerEmail, subject: `[Step 2] ผ่านการตรวจสอบแล้ว รออนุมัติ PR ${currentPR.pr_number}`, html_content: `<h3>เรียน กรรมการบริษัท (คุณเบญจมาศ),</h3><p>คุณจักรกฤษณ์ (หัวหน้า) ได้ตรวจสอบ PR เลขที่ <b>${currentPR.pr_number}</b> แล้ว</p><p>กรุณาพิจารณาอนุมัติขั้นตอนสุดท้าย: <a href="${adminLink}">คลิกที่นี่</a></p>` });
            alert('✅ บันทึกผลแล้ว ส่งต่อให้กรรมการเรียบร้อย!');
        } 
        
        // --- CASE 2: Manager (bestworld) กด ---
